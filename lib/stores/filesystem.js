...
class FilesystemStore {
  ...
  async putMetadata(bucket, key, metadata, md5) {
    ...
    try {
      await Promise.all([
        fs.writeFile(tempMetadataPath, JSON.stringify(json, null, 2)),
        md5 && fs.writeFile(tempMd5Path, md5),
      ]);
      await Promise.all([
        fs.rename(tempMetadataPath, metadataPath),
        md5 && fs.rename(tempMd5Path, md5Path),
      ]);
    } catch (err) {
      // clean up temporary files if any error occurs
      await Promise.all([
        fs.unlink(tempMetadataPath).catch(() => {}),
        md5 && fs.unlink(tempMd5Path).catch(() => {}),
      ]);
      throw err;
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Buffer.isBuffer(content)) {
        ...
        writeStream.on('finish', () => {
          fs.rename(tempPath, objectPath, (err) => {
            if (err) {
              reject(err);
            } else {
